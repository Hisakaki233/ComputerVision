...
def part_2b():
    ...
    sigma_mse = 2.5 # Define the value of sigma for the measurement exponential equation
...
def part_3():
    ...
    sigma_mse = 2.5  # Define the value of sigma for the measurement exponential equation
    sigma_dyn = 10  # Define the value of sigma for the particles movement (dynamics)
    alpha = 0.6  # Set a value for alpha
...
def part_4():
    ...
    num_particles = 1000  # Define the number of particles
    sigma_md = 5.0  # Define the value of sigma for the measurement exponential equation
    sigma_dyn = 1.0  # Define the value of sigma for the particles movement (dynamics)
    alpha = 1.00
...
def part_5():
    ...
    frame_list = {'1': [1, 56], '2': [1, 46], '3': [38, 71]}
    trackFrame = [0] * 3
    template_list = {}
    ...
    template_rect3 = {'x': 85, 'y': 215, 'w': 30, 'h': 30}
    ...
    for img in imgs_list:
        ...
        if algorithm == 'KF':
            ...
            if frame_num == 0:
                for key, value in template_rect_list.items():
            ...
            for key, value in template_list.items():

                corr_map = cv2.matchTemplate(frame, template_list[key], cv2.TM_SQDIFF)
                z_y, z_x = np.unravel_index(np.argmin(corr_map), corr_map.shape)
                ...
                maxframe = frame_list[key][-1]
                minframe = frame_list[key][0]
                frame_num = frame_num

                if frame_num == 0:
                    trackFrame[int(key) - 1] = ps5.KalmanFilter(template_rect_list[key]['x'], template_rect_list[key]['y'])

                if (frame_num <= maxframe) & (frame_num >= minframe):
                    x, y = trackFrame[int(key) - 1].process(z_x, z_y)
                    if True:  # For debugging, it displays every frame
        ...
        if algorithm == 'PF':
            ...
            # Overwrite template list
            template_rect1 = {'x': 70, 'y': 215, 'w': 45, 'h': 45}
            # template_rect2 = {'x': 280, 'y':200, 'w': 45, 'h': 45}
            template_rect2 = {'x': 265, 'y': 200, 'w': 75, 'h': 105}
            template_rect3 = {'x': 85, 'y': 215, 'w': 30, 'h': 30}
            ...
            for key, value in template_list.items():
                ...
                # Instantiate objects
                if (frame_num == 0):
                    trackFrame[int(key)-1] = ps5.AppearanceModelPF(frame, template, num_particles=num_particles, sigma_exp=sigma_mse,
                                                                   sigma_dyn=sigma_dyn, alpha=alpha,
                                                                   template_coords=template_rect_list[key])

                if (frame_num <= maxframe) & (frame_num >= minframe):
                    trackFrame[int(key)-1].process(frame)

                    if True:  # For debugging, it displays every frame
                        trackFrame[int(key)-1].render(frame)
...
def part_6():
    ...
    template_rect_kf = {'x': 80, 'y': 100, 'w': 45, 'h': 45}
    template_rect_pf = {'x': 260, 'y': 80, 'w': 45, 'h': 45}
    switchFrame = 60
    frame_num = 0
    pf = None
    ...
    for img in imgs_list:
        ...
        if frame_num <= switchFrame:
            noise = {'x': 3.5, 'y': 3.5}

            if frame_num == 0:
                template = frame[template_rect_kf['y']:
                                 template_rect_kf['y'] + template_rect_kf['h'],
                                 template_rect_kf['x']:
                                 template_rect_kf['x'] + template_rect_kf['w']]

            corr_map = cv2.matchTemplate(frame, template, cv2.TM_SQDIFF)
            z_y, z_x = np.unravel_index(np.argmin(corr_map), corr_map.shape)

            z_w = template_rect_kf['w']
            z_h = template_rect_kf['h']
            ...
            kf = ps5.KalmanFilter(template_rect_kf['x'], template_rect_kf['y'])
            x, y = kf.process(z_x, z_y)
            ...
            # Overwrite template list
            frame_num = frame_num
            if frame_num == switchFrame + 1:

                template = frame[template_rect_pf['y']:
                                 template_rect_pf['y'] + template_rect_pf['h'],
                                 template_rect_pf['x']:
                                 template_rect_pf['x'] + template_rect_pf['w']]

                if 'template' in save_frames:
                    cv2.imwrite(save_frames['template'], template)

                # Instantiate objects
                pf = ps5.AppearanceModelPF(frame, template, num_particles=num_particles,
                                           sigma_exp=sigma_mse,
                                           sigma_dyn=sigma_dyn, alpha=alpha,
                                           template_coords=template_rect_pf)

...
def helper_for_part_6(video_name, myAds, fps, frame_ids, output_prefix, counter_init):

    video = os.path.join(VID_DIR, video_name)
    ads = os.path.join(VID_DIR, myAds)

    image_gen = ps3.video_frame_generator(video)
    my_image_gen = ps3.video_frame_generator(ads)
...
def part_1():
    ...
    for img_in, img_out in zip(input_images, output_images):
        ...
        marker_positions = ps3.find_markers(scene, template) # template
...
def part_2():
    ...
    for img_in, img_out in zip(input_images, output_images):
        ...
        markers = ps3.find_markers(scene, template) # template
...
def part_3():
    ...
    for img_in, img_out in zip(input_images, output_images):
        ...
        # Open image and identify the four marker positions
        scene = cv2.imread(os.path.join(IMG_DIR, img_in))
        markers = ps3.find_markers(scene)
